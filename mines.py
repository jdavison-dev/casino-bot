...
class MineTile(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if index in self.view.bombs:
            ...
            await interaction.response.edit_message(content=f"💥 You hit a mine! Game over, {interaction.user.mention}.", view=self.view)
        else:
            self.style = discord.ButtonStyle.success
            self.label = "✅"
            self.view.safe_reveals += 1
            self.view.multiplier = round(1 + self.view.safe_reveals * 0.2, 2)
            await interaction.response.edit_message(content=f"{interaction.user.mention}, Multiplier: x{self.view.multiplier} — Type `cash out` to stop or keep playing!", view=self.view)
...
async def play_mines(ctx, bet, mines, load_data, save_data, bot):
    ...
    message = await ctx.respond(f"{ctx.author.mention} started a game of Mines! Multiplier: x1.0 — Click tiles or type `cash out` to stop.", view=view)
    ...
    try:
        ...
        while not view.game_over:
            ...
            if msg.content.lower() == "cash out" and not view.game_over:
                ...
                # Optionally disable buttons after cashout
                for item in view.children:
                    item.disabled = True
                await msg.channel.fetch_message((await message).id).then(lambda m: m.edit(view=view))
        ...
        if not view.game_over:
            ...
            for item in view.children:
                item.disabled = True
            await (await message).edit(view=view)
